...
for (let pt of points) {
  ...
  if (dist(prevPt.x, prevPt.y, pt.x, pt.y) > 20) {
    endShape(CLOSE)
    beginShape()
  }
...
## Working with Images: image copies
...
For this, it's helpful to use the [image copy](https://p5js.org/reference/#/p5/image) functionality. This function copies a rectangular region from an image onto the canvas. This can be used to copy a row, column, or grid square.

> Note: There is a copy() function which does the same thing but is much slower. Do not use that.
...
There are a lot of other interesting ways to change your image copy to make new, interesting things! You can skip certain rows in the destination image, or warp-scale the resulting image if the source width/height doesn't match the destination width/height. 
...
Do you want things more cube-y? Curved? blinder'd? These are all attributes that you can affect using an image copy. And I've only covered one way to read images (row by row)! Try it out with columns or grid squares and see what you can make :)
